...
<div class="list">
    ...
    <div class="list__tabs">
        <a class="list__tab {{ $status === 'pending' ? 'is-active' : '' }}" href="{{ route('stamp_correction_request.list', ['status' => 'pending']) }}">
            承認待ち
        </a>

        <a class="list__tab {{ $status === 'approved' ? 'is-active' : '' }}" href="{{ route('stamp_correction_request.list', ['status' => 'approved']) }}">
            承認済み
        </a>
    </div>
            ...
            <tbody class="list__tbody">
                @forelse ($requests as $req)
                    <tr class="list__tr {{ $loop->last ? 'list__tr--last' : '' }}">
                        ...
                        <td class="list__td">{{ \Carbon\Carbon::parse($req->attendance->work_date)->format('Y/m/d') }}</td>
                        <td class="list__td">{{ $req->requested_note }}</td>
                        <td class="list__td">{{ $req->created_at->format('Y/m/d') }}</td>
                        <td class="list__td">
                            <a class="list__detail-link" href="{{ route('attendance.detail', ['id' => $req->attendance->id]) }}">
                                詳細
                            </a>
                        </td>
                    </tr>
